...
$shape_id = $_POST['shape'];
  if (!isset($shape_id)) {
    $sql_diamond = mysqli_query($con,"SELECT * FROM `diamonds` WHERE `diamond_shape_id` = '17' AND diamond_status NOT IN ('Deleted', 'Invoiced') AND diamond_lot_no LIKE 'M%' AND diamond_type = 'MatchingPair'");
  } else {
    $sql_diamond = mysqli_query($con,"SELECT * FROM `diamonds` WHERE `diamond_shape_id` = '".$shape_id."' AND diamond_status NOT IN ('Deleted', 'Invoiced') AND diamond_lot_no LIKE 'M%' AND diamond_type = 'MatchingPair'");
  }
 while($row_diamond = mysqli_fetch_assoc($sql_diamond)){
     ...
     if ($row_diamond['diamond_status'] == 'On Consignment') {
       ...
       <tr style='font-size:16px; background-color: #F1C4C0;'>
         ...
         <td>".$row_diamond['diamond_size_available']."</td>
         <td>".$row_diamond['diamond_size']."</td>
         <td>".$row_diamond['diamond_pcs']."</td>
